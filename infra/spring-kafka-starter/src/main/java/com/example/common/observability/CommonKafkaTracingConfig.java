...
/**
 * Common Kafka tracing configuration that propagates trace context through Kafka headers.
 * This can be used across all microservices for consistent distributed tracing.
 */
@Configuration
public class CommonKafkaTracingConfig {
    ...
    /**
     * Utility method to extract tracing headers for manual processing
     */
    public static void extractTracingHeaders(Headers headers) {
        Header traceIdHeader = headers.lastHeader(TRACE_ID_HEADER);
        if (traceIdHeader != null) {
            String traceId = new String(traceIdHeader.value(), StandardCharsets.UTF_8);
            MDC.put("traceId", traceId);
